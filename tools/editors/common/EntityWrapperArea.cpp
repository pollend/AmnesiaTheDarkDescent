								...
								bool abIsSelected, const cColor& aHighlightCol, const cColor& aDisabledCol)
{
	...
	options.m_transform = cMath::ToForgeMatrix4(mtxRT);
	...
	if(pType->GetDrawAsSphere())
	{
		...
		apFunctions->DebugDrawSphere(Vector3(0), vHalfScale.x, cMath::ToForgeVec4(col), options);
	}
	else
	{
		apFunctions->DebugDrawBoxMinMax(cMath::ToForgeVec3(vHalfScale*-1), cMath::ToForgeVec3(vHalfScale), cMath::ToForgeVec4(col), options);
	}
	...
	if(pType->GetShowOrientation())
	{
		...
		for(int i=0;i<3;++i)
		{
			...
			vDir = vDir*vLength;
			col.v[i] = 1;
	        apFunctions->DebugDrawLine(Vector3(0) , cMath::ToForgeVec3(vDir), cMath::ToForgeVec4(col), options);
			apFunctions->DrawPyramid(cMath::ToForgeVec3(vDir), cMath::ToForgeVec3(vDir+ vDir*0.1f), 0.025f, cMath::ToForgeVec4(col), options);
		}
	}
