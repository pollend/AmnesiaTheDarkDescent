	...
	cSimpleUpdate() : iUpdateable("Simple3D")
	{
		...
		m_postSolidDraw = cViewport::PostSolidDraw::Handler([&](cViewport::PostSolidDrawPacket& payload) {
			...
			if(gbDrawSepperateObjects)
			{
				...
				while(meshIt.HasNext())
				{
					...
					for(int i=0; i<pEntity->GetSubMeshEntityNum(); ++i)
					{
						...
						options.m_transform = cMath::ToForgeMatrix4(*pSubEnt->GetModelMatrix(NULL));
			...
			if(gbDrawOcclusionGfxInfo)
			{
				...
				for(int i=lNum-1; i>=0; --i)
				{
					...
					DebugDraw::DebugDrawOptions options;
					options.m_transform = cMath::ToForgeMatrix4(*pObject->GetModelMatrix(payload.m_frustum));
		...
		m_postTestSolidDraw = cViewport::PostSolidDraw::Handler([&](cViewport::PostSolidDrawPacket& payload) {
			...
			for(int i=0; i<lNum; ++i)
			{
				...
				DebugDraw::DebugDrawOptions options;
				options.m_transform = cMath::ToForgeMatrix4(*pObject->GetModelMatrix(nullptr));
