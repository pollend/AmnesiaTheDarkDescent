...
void cLuxMainMenu::RenderBlurTexture()
{
	...
	auto requestBlur = [&](Image& input){
		...
		auto image = blurTargets[1].GetImage(0);
		{
			...
			GraphicsContext::ViewConfiguration viewConfig {blurTargets[0]};
...

		{
			...
			cMatrixf projMtx;
			graphicsContext.ScreenSpaceQuad(layoutStream, projMtx, mvScreenSize.x, mvScreenSize.y);
			GraphicsContext::ViewConfiguration viewConfig {blurTargets[1]};
			viewConfig.m_projection = projMtx;
			viewConfig.m_viewRect = {0, 0, image->GetWidth(), image->GetHeight()};
			bgfx::ViewId view = graphicsContext.StartPass("Blur Pass 2", viewConfig);
