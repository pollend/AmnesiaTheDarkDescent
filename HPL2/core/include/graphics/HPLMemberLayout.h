...
template <typename TParameter> class HPLMemberLayout  {
public:
  using TParamType = TParameter;

  template <typename TStruct>
  HPLUniformLayout<TParameter, TStruct> &addUniform(TStruct TParameter::*field) {
    return *static_cast<HPLUniformLayout<TParameter, TStruct> *>(
        _uniform
            .emplace_back(
                std::make_unique<HPLUniformLayout<TParameter, TStruct>>(field))
            .get());
  }



  HPLSamplerLayout<TParameter> &addSampler(HPLSampler TParameter::*field) {
    return *_samplers.emplace_back(HPLSamplerLayout<TParameter>(field));
  }
...
//
private:
  std::vector<HPLSamplerLayout<TParameter>> _samplers;
