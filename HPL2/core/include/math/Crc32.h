...
namespace hpl::math {
    ...
    constexpr void Crc32::Update(const std::string_view value) {
        for(auto& c: value) {
            m_value = detail::Update(m_value, c);
        }
    }
    ...
    constexpr void Crc32::Update(const uint32_t value) {
        m_value = detail::Update(m_value, value);
    }
    constexpr void Crc32::Update(const Uuid& value) {
        for(auto& c: value.m_data) {
            m_value = detail::Update(m_value, c);
        }
    }
    constexpr Crc32::Crc32(std::string_view value) {
        Update(value);
    }
} // namespace hpl::math
