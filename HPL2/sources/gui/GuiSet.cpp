...
namespace hpl {
    ...
    namespace gui {
        ...
        void InitializeGui(ForgeRenderer& pipeline) {
...

            {
                ...
                auto materialBlendFunc =
                    [](BlendConstant srcColor, BlendConstant destColor, BlendConstant srcAlpha, BlendConstant destAlpha) {
                ...
                blendStateDesc[eGuiMaterial_PremulAlpha] =
                    materialBlendFunc(BC_ONE, BC_ONE_MINUS_SRC_ALPHA, BC_ONE, BC_ONE_MINUS_SRC_ALPHA);
                ...
                blendStateDesc[eGuiMaterial_FontNormal] =
                    materialBlendFunc(BC_SRC_ALPHA, BC_ONE_MINUS_SRC_ALPHA, BC_SRC_ALPHA, BC_ONE_MINUS_SRC_ALPHA);
                blendStateDesc[eGuiMaterial_Alpha] =
                    materialBlendFunc(BC_SRC_ALPHA, BC_ONE_MINUS_SRC_ALPHA, BC_SRC_ALPHA, BC_ONE_MINUS_SRC_ALPHA);
    ...
    bool cGuiRenderObjectCompare::operator()(const cGuiRenderObject& aObjectA, const cGuiRenderObject& aObjectB) const {
        ...
        // Material
        eGuiMaterial pMaterialA =
            aObjectA.mpCustomMaterial != eGuiMaterial::eGuiMaterial_LastEnum ? aObjectA.mpCustomMaterial : aObjectA.mpGfx->m_materialType;
        eGuiMaterial pMaterialB =
            aObjectB.mpCustomMaterial != eGuiMaterial::eGuiMaterial_LastEnum ? aObjectB.mpCustomMaterial : aObjectB.mpGfx->m_materialType;
    ...
    cGuiGlobalShortcut::cGuiGlobalShortcut(
        int alKeyModifiers, eKey aKey, iWidget* apWidget, eGuiMessage aMessage, bool abBypassVisibility, bool abBypassEnabled) {
        mbEnabled = true;
    ...
    bool cGuiGlobalShortcut::ProcessKeyPress(const cKeyPress& aKey) {
        if (IsEnabled() == false || mKey.mlModifier != aKey.mlModifier || mKey.mKey != aKey.mKey)
            return false;
    ...
    cGuiGlobalShortcut* cGuiSet::AddGlobalShortcut(
        int alKeyModifiers, eKey aKey, iWidget* apWidget, eGuiMessage aMessage, bool abBypassVisibility, bool abBypassEnabled) {
        cGuiGlobalShortcut* pShortcut =
            hplNew(cGuiGlobalShortcut, (alKeyModifiers, aKey, apWidget, aMessage, abBypassVisibility, abBypassEnabled));
    ...
    void cGuiSet::Draw(const ForgeRenderer::Frame& frame, cFrustum* apFrustum) {
        ...
        if (mbIs3D && apFrustum) {
            ...
            // Also scale to size
            cVector3f vPreScale =
                cVector3f(mv3DSize.x / mvVirtualSize.x, -mv3DSize.y / mvVirtualSize.y, mv3DSize.z / (mfVirtualMaxZ - mfVirtualMinZ));
        ...
        while (it != m_setRenderObjects.end()) {
            ...
            do {
                ...
                if (object.mbRotated) {
                    for (int i = 0; i < 4; ++i) {
                        ...
                        reinterpret_cast<gui::PositionTexColor*>(
                            vertexUpdateDesc.pMappedData)[vertexBufferOffset + (vertexBufferIndex++)] = {
            ...
            } while (pTexture == pLastTexture && materialType == pLastMaterial && pClipRegion == pLastClipRegion);
    ...
    void cGuiSet::DrawFont(
        iFontData* apFont, const cVector3f& avPos, const cVector2f& avSize, const cColor& aColor, const wchar_t* fmt, ...) {
        va_list ap;
        if (fmt == NULL)
            return;
        va_start(ap, fmt);
    ...
    cWidgetWindow* cGuiSet::CreateWidgetWindow(
        tWidgetWindowButtonFlag alFlags,
    ...
    cWidgetFrame* cGuiSet::CreateWidgetFrame(
        const cVector3f& avLocalPos,
        const cVector2f& avSize,
        bool abDrawFrame,
        iWidget* apParent,
    ...
    cWidgetButton* cGuiSet::CreateWidgetButton(
        const cVector3f& avLocalPos,
        const cVector2f& avSize,
        const tWString& asText,
        iWidget* apParent,
    ...
    cWidgetLabel* cGuiSet::CreateWidgetLabel(
        const cVector3f& avLocalPos, const cVector2f& avSize, const tWString& asText, iWidget* apParent, const tString& asName) {
    ...
    cWidgetSlider* cGuiSet::CreateWidgetSlider(
    ...
    cWidgetTextBox* cGuiSet::CreateWidgetTextBox(
        const cVector3f& avLocalPos,
        const cVector2f& avSize,
        const tWString& asText,
        iWidget* apParent,
    ...
    cWidgetCheckBox* cGuiSet::CreateWidgetCheckBox(
        const cVector3f& avLocalPos, const cVector2f& avSize, const tWString& asText, iWidget* apParent, const tString& asName) {
    ...
    cWidgetImage* cGuiSet::CreateWidgetImage(
    ...
    cWidgetListBox* cGuiSet::CreateWidgetListBox(
        const cVector3f& avLocalPos, const cVector2f& avSize, iWidget* apParent, const tString& asName) {
    ...
    cWidgetMultiPropertyListBox* cGuiSet::CreateWidgetMultiPropertyListBox(
        const cVector3f& avLocalPos, const cVector2f& avSize, iWidget* apParent, const tString& asName) {
    ...
    cWidgetComboBox* cGuiSet::CreateWidgetComboBox(
        const cVector3f& avLocalPos, const cVector2f& avSize, const tWString& asText, iWidget* apParent, const tString& asName) {
    ...
    cWidgetMenuItem* cGuiSet::CreateWidgetMenuItem(
        const cVector3f& avLocalPos, const cVector2f& avSize, const tWString& asText, iWidget* apParent, const tString& asName) {
    ...
    cWidgetContextMenu* cGuiSet::CreateWidgetContextMenu(
        const cVector3f& avLocalPos, const cVector2f& avSize, const tWString& asText, iWidget* apParent, const tString& asName) {
    ...
    cWidgetMainMenu* cGuiSet::CreateWidgetMainMenu(iWidget* apParent, const tString& asName) {
    ...
    cWidgetTabLabel* cGuiSet::CreateWidgetTabLabel(
        const cVector3f& avLocalPos, const cVector2f& avSize, const tWString& asText, iWidget* apParent, const tString& asName) {
    ...
    cWidgetTab* cGuiSet::CreateWidgetTab(
        const cVector3f& avLocalPos, const cVector2f& avSize, const tWString& asText, iWidget* apParent, const tString& asName) {
    ...
    cWidgetTabFrame* cGuiSet::CreateWidgetTabFrame(
        const cVector3f& avLocalPos,
        const cVector2f& avSize,
        const tWString& asText,
        iWidget* apParent,
    ...
    cWidgetGroup* cGuiSet::CreateWidgetGroup(
        const cVector3f& avLocalPos, const cVector2f& avSize, const tWString& asText, iWidget* apParent, const tString& asName) {
    ...
    cWidgetDummy* cGuiSet::CreateWidgetDummy(const cVector3f& avLocalPos, iWidget* apParent, const tString& asName) {
    ...
    cGuiPopUpMessageBox* cGuiSet::CreatePopUpMessageBox(
        ...
        cGuiPopUpMessageBox* pMessageBox =
            hplNew(cGuiPopUpMessageBox, (this, asLabel, asText, asButton1, asButton2, apCallbackObject, apCallback));
    ...
    cGuiPopUpFilePicker* cGuiSet::CreatePopUpSaveFilePicker(
        tWString& asFileName,
        const tWString& asCategory,
        const tWString& asFilter,
        const tWString& asStartPath,
        bool abShowHidden,
        void* apCallbackObject,
        tGuiCallbackFunc apCallback,
        const tWString& asStartFilename) {
        cGuiPopUpFilePicker* pPicker = hplNew(
            cGuiPopUpFilePicker,
            (this, mpSkin, eFilePickerType_Save, asStartPath, abShowHidden, apCallbackObject, apCallback, asStartFilename));
        pPicker->Init();
     ...
     * \return
     */
    cGuiPopUpFilePicker* cGuiSet::CreatePopUpLoadFilePicker(
        tWStringVec& avFileList,
        bool abAddAllFilesFilter,
        const tWString& asStartPath,
        bool abShowHidden,
        void* apCallbackObject,
        tGuiCallbackFunc apCallback) {
        cGuiPopUpFilePicker* pPicker =
            hplNew(cGuiPopUpFilePicker, (this, mpSkin, eFilePickerType_Load, asStartPath, abShowHidden, apCallbackObject, apCallback));
        pPicker->Init();
    ...
    cGuiPopUpColorPicker* cGuiSet::CreatePopUpColorPicker(
        cColor* apDestColor, const cVector3f& avPos, void* apCallbackObject, tGuiCallbackFunc apCallback) {
    ...
    void cGuiSet::ShowContextMenu(cWidgetContextMenu* apMenu, const cVector3f& avPosition) {
        ...
        if (apMenu->GetParentItem() == NULL || apMenu->GetParentMenu()->GetType() == eWidgetType_MainMenu) {
    ...
    void cGuiSet::DrawTextFromCharArry(
        const wchar_t* apString,
        iFontData* apFont,
        const cVector2f& avSize,
        ...
        while (apString[lCount] != 0) {
            ...
            if (lGlyphNum < apFont->GetFirstChar() || lGlyphNum > apFont->GetLastChar()) {
                lCount++;
                continue;
            }
...
#define kLogRender (false)
    ...
    bool cGuiSet::OnMouseMove(const cGuiMessageData& aData) {
        ...
        for (; it != mlstWidgets.end(); ++it) {
            ...
            if (pWidget->PointIsInside(mvMousePos, false)) {
                if (pWidgetUnderMouse == NULL || pWidgetUnderMouse->IsConnectedTo(pWidget) || pWidget->IsConnectedTo(pWidgetUnderMouse)) {
                    ...
                    if (pWidget->GetMouseIsOver() == false) {
                        ...
                        if (pWidget->HasFocusNavigation() && pWidget->IsVisible() && pWidget->IsEnabled()) {
                            ...
                            if (pOldFocus == NULL ||
                                (pOldFocus->GetParent() == pWidget->GetParent() && pOldFocus->GetSet() == pWidget->GetSet())) {
    ...
    bool cGuiSet::OnUIArrowPress(const cGuiMessageData& aData) {
        ...
        if (mpFocusedWidget == NULL || mpFocusedWidget->HasFocusNavigation() == false && mpFocusedWidget != mpDefaultFocusNavWidget) {
        ...
        if (bRet == false) {
            ...
            if (pNextWidget && pNextWidget->IsEnabled() && pNextWidget->IsVisible() &&
    ...
    bool cGuiSet::DrawMouse(iWidget* apWidget, const cGuiMessageData& aData) {
        if (HasFocus() && mbDrawMouse && mpGfxCurrentPointer) {
            DrawGfx(
                mpGfxCurrentPointer, cVector3f(mvMousePos.x, mvMousePos.y, mfMouseZ), mpGfxCurrentPointer->GetImageSize(), cColor(1, 1));
        }
        ...
        if (mpCurrentToolTipWidget == NULL || mpCurrentToolTipWidget->IsVisible() == false) {
            ...
            if (mfToolTipTimer >= mfToolTipTimeToPopUp) {
                if (mpFrameToolTip->IsVisible() == false) {
                    ...
                    float fTextLength = mpLabelToolTip->GetDefaultFontType()->GetLength(mvFontSize, sTipText.c_str()) + 3 + 3;
                    ...
                    cVector2f vToolTipSize = cVector2f(cMath::Min(fTextLength, fMaxTextLength), lRows * (mvFontSize.y + 2) - 2 + 3 + 3);
