...
namespace hpl {
    ...
    void DrawPacket::cmdBindBuffers(Cmd* cmd, ForgeRenderer::CommandResourcePool* resourcePool, DrawPacket* packet) {
        ...
        case DrawPacketType::DrawGeometryset: {
            for(size_t i = 0; i < packet->m_unified.m_numStreams; i++) {
                ShaderSemantic semantic = hplToForgeShaderSemantic(packet->m_unified.m_elements[i]);
            ...
            cmdBindVertexBuffer(cmd, packet->m_unified.m_numStreams, vbBuffer.data(), vbStride.data(), vbOffsets.data());
